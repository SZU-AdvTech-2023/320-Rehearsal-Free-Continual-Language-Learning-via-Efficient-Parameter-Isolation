...
class Trainer:
    ...
    def new_task(self, train_dataset, val_dataset, num_labels):
        ...
        if self.model.num_tasks>0 and self.args.prompt_fusion_mode == 'SMD':
            loader = DataLoader(
            ...
            scores_over_tasks = []
            with torch.no_grad():
                for _, batch in enumerate(tqdm(loader, desc="Get sample Mahalanobis distance")):
                    input_ids, attention_mask, label = batch
                    input_ids = input_ids.cuda()
                    attention_mask = attention_mask.cuda()
                    scores_over_tasks = self.model(
                        input_ids=input_ids,
                        attention_mask=attention_mask,
                        get_scores=True,
                        by_prefix=self.args.query_mode == "mahalanobis",
                    )
            mmean = torch.max(torch.Tensor([score.mean() for score in scores_over_tasks]))
            mvar = torch.max(torch.Tensor([score.var() for score in scores_over_tasks]))
            theta = mmean / mvar
            darr = torch.Tensor([1/(score.mean()+score.var()*theta) for score in scores_over_tasks])
            thesum = torch.sum(darr)
            topk = int(self.model.num_tasks/2)+1
            new_prompt_data_1 = torch.sum(torch.cat([
                (d/thesum*prompt).unsqueeze(dim=0)
                for i, (d, prompt) in enumerate(zip(darr, self.model.prompts))
                if i in torch.topk(darr, topk).indices]),
                dim=0)

            bk_prompt_fusion_mode = self.model.args.prompt_fusion_mode
            self.model.args.prompt_fusion_mode = 'mean'
            new_prompt_data_2 = self.model.get_prompts_data()
            self.model.args.prompt_fusion_mode = bk_prompt_fusion_mode

            import math
            alpha = 1/math.log2(self.args.n_per_class) \
                if self.args.n_per_class > 2 \
                else 0.5

            new_prompt_data = alpha * new_prompt_data_1 + \
                (1-alpha) * new_prompt_data_2
            self.model.prompts.append(torch.nn.Parameter(new_prompt_data, requires_grad=True))

        # expand classifier and prefix

        self.model.new_task(num_labels)
        self.model.prompts[-1].requires_grad = True
